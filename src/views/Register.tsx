...
interface RegisterProps {
  onSwitch: () => void;
  onRegister: () => void;
  initialAccountType?: "employee" | "hr" | "company";
}
...
export default function Register({
  ...
  const [accountType, setAccountType] = useState<"employee" | "hr" | "company">(
    initialAccountType,
  );
  ...
  return (
    <div className="min-h-screen bg-surface flex flex-col md:flex-row antialiased overflow-hidden">
      <div className="w-full md:w-1/2 bg-primary relative flex flex-col justify-between p-12 lg:p-24 overflow-hidden min-h-[400px]">
        ...
        >
          <div className="h-10 w-10 rounded-xl bg-white/10 backdrop-blur-md flex items-center justify-center text-white font-bold border border-white/20">
            <LayoutGrid size={24} />
          </div>
        ...
        <div className="relative z-10">
          ...
          >
            Streamline your attendance tracking, analyze workforce trends, and
            maintain perfect operational oversight from a single, unified
            dashboard.
          </motion.p>
        </div>
        <motion.div
          ...
          className="relative z-10 hidden md:flex items-center gap-6 text-white/50 text-[10px] font-black uppercase tracking-[0.2em]"
        >
          ...
          <header>
            ...
            <p className="text-on-surface-variant font-medium">
              {accountType === "company"
                ...
                : "Enter your details to request access to the system."}
            </p>
          </header>
          <form
            className="space-y-8"
          ...
          >
            <div className="space-y-4">
              ...
              <div className="flex p-1.5 bg-surface-container rounded-2xl border border-outline-variant/20 shadow-inner">
                {["employee", "hr", "company"].map((type) => (
                  <button
                    key={type}
                    type="button"
                    onClick={() => setAccountType(type as any)}
                    className={cn(
                      "flex-1 py-3 text-center text-[10px] sm:text-xs font-black uppercase tracking-widest rounded-xl transition-all duration-300",
                      accountType === type
            ...
            <AnimatePresence>
              {accountType === "hr" && (
                <motion.div
                  ...
                  className="bg-surface-container-low border border-outline-variant/30 p-6 rounded-3xl flex items-start gap-4 shadow-sm overflow-hidden"
                >
                  <div className="bg-primary/5 p-2 rounded-xl text-primary shrink-0">
                    <Info size={20} />
                  </div>
                  <div className="text-sm text-on-surface-variant leading-relaxed">
                    <strong className="text-primary font-black block mb-1 uppercase tracking-widest text-[10px]">
                      Approval Required
                    </strong>
              ...
              {accountType === "company" && (
                ...
                >
                  ...
                  <div className="text-sm text-on-surface-variant leading-relaxed">
                    ...
                    Registering a company creates a new organization and assigns
                    you the Owner role.
                  </div>
                </motion.div>
              )}
            ...
            <div className="space-y-6">
              {accountType === "company" && (
                <div className="space-y-2">
                  <label
                    className="text-[11px] font-black text-on-surface-variant uppercase tracking-widest pl-1"
                    htmlFor="companyName"
                  >
                    Company Name
                  </label>
                  <div className="relative">
                    ...
                    <input
                      id="companyName"
                      type="text"
                      className="w-full bg-slate-50 border border-outline-variant/30 rounded-2xl pl-12 pr-6 py-4 font-medium text-primary focus:ring-2 focus:ring-primary outline-none transition-all placeholder:text-on-surface-variant/40"
            ...
            <button
              type="submit"
              className="w-full bg-primary text-white font-black uppercase tracking-widest text-sm py-5 rounded-[2rem] shadow-2xl shadow-primary/20 hover:bg-primary-container active:scale-[0.98] transition-all flex items-center justify-center gap-3"
            >
              {accountType === "company"
                ? "Register Company"
                : "Request Account"}{" "}
